...
/**
 * Mobile-only swipeable team carousel — same scroll-snap mechanics as
 * PeopleCarousel (About "The People"), but each card also shows the member's
 * career bullets so no information is lost vs the desktop TeamGrid.
 */
export function TeamCarousel({
  ...
  return (
    <div className="mt-8">
      ...
      >
        {members.map((m) => {
          const bio = m.bio.length > 0 ? m.bio : null;
          return (
            <div
              key={m.name}
              className="flex shrink-0 basis-[82%] snap-start flex-col sm:basis-[55%]"
            >
              <div className="relative aspect-[41/50] overflow-hidden bg-[#edeff2]">
                <SlotImage
                  slots={slots}
                  slotKey={m.slotKey}
              ...
              <div className="mt-3 flex flex-wrap items-baseline gap-x-3 gap-y-0.5">
                <p className="text-lg font-medium tracking-[-0.22px] text-foreground">
                  {m.name}
                </p>
              ...
              {bio ? (
                <ul className="mt-3 list-disc space-y-1 pl-4 text-[12px] font-medium leading-snug tracking-[-0.1px] text-secondary">
                  {bio.map((b, i) => (
                    <li key={i}>{b}</li>
                  ))}
                </ul>
              ) : (
                <span className="mt-3 text-[12px] font-medium tracking-[-0.1px] text-tertiary">
                  {bioFallback}
                </span>
              )}
            </div>
